...
template <typename T>
constexpr Matrix3<T> Quaternion(const T w, const T x, const T y, const T z)
{
    Matrix<T, 3, 3> R;
...
template <typename T>
constexpr Matrix3<T> Quaternion(const std::array<T, 4>& wxyz)
{
    return Quaternion(wxyz[0], wxyz[1], wxyz[2], wxyz[3]);
}

template <typename T>
constexpr std::array<T, 4> RotToQuaternion(const Matrix3<T>& R)
{
    std::array<T, 4> wxyz;
    const auto trace = Trace(R);
    if(trace > 0.)
    {
        const T s = 0.5 / sqrt(trace + 1.0);
        wxyz[0] = 0.25 / s;
    ...
    else
    {
        ...
        {
            const T s = 2.0 * sqrt(1.0 + R.Get(0, 0) - R.Get(1, 1) - R.Get(2, 2));
        ...
        {
            const T s = 2.0 * sqrt(1.0 + R.Get(1, 1) - R.Get(0, 0) - R.Get(2, 2));
        ...
        {
            const T s = 2.0 * sqrt(1.0 + R.Get(2, 2) - R.Get(0, 0) - R.Get(1, 1));
...
constexpr Matrix3<T> Vec6ToRot(const Vector6<T>& Rep)
{
    ...
    const Vector3<T> a1 = Extract<Vector3<T>, 0, 0>(Rep);
    const Vector3<T> a2 = Extract<Vector3<T>, 3, 0>(Rep);

    const Vector3<T> b1 = Norm(a1);
    const Vector3<T> b2 = Norm(Add(a2, Matmul(-1. * Sum(Dot(b1, a2)), b1)));
    const Vector3<T> b3 = Cross(b1, b2);
...
public:
    ...
    // todo: remove ToFull
    constexpr Rotation<T> Inverse() const { return Rotation<T>(Transpose(mMatrix)); }
