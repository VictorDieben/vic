...
public:
	...
	constexpr static std::size_t GetRows()
	{
		return Rows;
	}
	constexpr static std::size_t GetColumns()
	{
		return Columns;
	}
...
// 2d cross product
template <typename T>
requires ConceptVector<T> &&(T::GetRows() == 2) constexpr auto Cross(const T& vec1, const T& vec2)
{
...
requires ConceptVector<T> &&
	(T::GetRows() == 3) constexpr auto Cross(const Vector3<T>& vec1, const Vector3<T>& vec2)
{
...
requires ConceptVector<TMat1> && ConceptVector<TMat2> &&
	HasSameShape<TMat1, TMat2>::value constexpr auto
