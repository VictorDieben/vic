...
struct Edge
{
	...
	Edge(const VertexIdType source, const VertexIdType sink, const EdgeIdType id)
		...
		, mId(id)
	{ }
	Edge(const VertexIdType source,
		 const VertexIdType sink,
		 const EdgeIdType id,
...
public:
	...
	VertexType& GetVertex(const VertexIdType id)
	{
		return mVertices.at(id);
	} // id is also index
	EdgeType& GetEdge(const EdgeIdType id)
	{
		return mEdges.at(id);
	} // id is also index
...
public:
	...
	TensorVertex(const TensorVertexId& id)
	{
		FromId(id);
	}

	TensorVertexId ToId() const
	{
		return {};
	}
