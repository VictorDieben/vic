...
TEST(TestLinalg, TestMatmul)
{
	...
	EXPECT_TRUE(
		(std::is_same_v<double,
						decltype(Matmul(Identity<double, 1>{}, Identity<double, 1>{}))::DataType>));
	EXPECT_TRUE(
		(std::is_same_v<double,
						decltype(Matmul(Identity<double, 1>{}, Identity<float, 1>{}))::DataType>));
	EXPECT_TRUE(
		(std::is_same_v<double,
						decltype(Matmul(Identity<double, 1>{}, Identity<int, 1>{}))::DataType>));
	EXPECT_TRUE(
		(std::is_same_v<int, decltype(Matmul(Identity<int, 1>{}, Identity<int, 1>{}))::DataType>));
}
