...
TEST(Kinematics, EulerAngles)
{
    ...
    // * Intrinsic means that subsequent rotations are dedined in the new intermediade frames
    ...
    static constexpr auto sqrtTwo = 1.41421356237; // todo: replace with std::sqrt once it is constexpr
    static constexpr auto sqrtTwoInv = 1. / sqrtTwo;

    Matrix3<double> euler = EulerAngles(0., std::numbers::pi / 4., std::numbers::pi / 2.);
    Matrix3<double> matIntrinsic(0., -sqrtTwoInv, sqrtTwoInv, 1., 0., 0., 0., sqrtTwoInv, sqrtTwoInv);
    Matrix3<double> matExtrinsic(0., -1., 0., sqrtTwoInv, 0., sqrtTwoInv, -sqrtTwoInv, 0., sqrtTwoInv);
    ...
    // TODO: for Sequential 'sxyz', or: rol alpha around x -> pitch beta around (new) y -> yaw gamma around (newest) z,
...
TEST(Kinematics, CartesianRobot)
{
    ...
    const auto rootId = robot.AddJoint(std::nullopt, {}, stx); // x axis
    const auto firstId = robot.AddJoint(rootId, {}, sty); // y axis
    const auto secondId = robot.AddJoint(firstId, {}, stz); // z axis
